...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("আমাকে add করার জন্য ধন্যবাদ", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nالسلام عليكم ورحمه الله وبركاته 
<------------------------------>  
...
🟣Facebook Account Link: https://www.facebook.com/profile.php?

🔵প্রেম ভালোবাসা একটি মিথ্যা মায়া এটা থেকে যত দূরে থাকবে ততই মঙ্গল`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
